...
import {
  ...
  ChevronDown, Shield, Lock, Briefcase, ClipboardList, Plus, AlertCircle, Clock, Filter
} from "lucide-react";
...
interface AfastamentoAPI { id_afastamento: number; id_mandato: number; motivo: string; data_inicio: string; data_fim: string; }
interface UnidadeAPI { id_unidade: number; id_unidade_pai: number | null; sigla: string; nome: string; }
...
const Modal = ({ title, onClose, children, maxW = "max-w-2xl" }: { title: string, onClose: () => void, children: React.ReactNode, maxW?: string }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
    <div className={`bg-white rounded-2xl shadow-xl w-full ${maxW} border border-slate-200 overflow-y-auto max-h-[90vh] animate-in zoom-in-95 duration-200`}>
      <div className="sticky top-0 bg-white z-10 flex items-center justify-between px-6 py-4 border-b border-slate-100">
        ...
        <button onClick={onClose} className="p-1 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-lg"><X size={16} /></button>
      </div>
    ...
    <div className="max-w-7xl mx-auto p-6 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex border-b border-slate-200 overflow-x-auto no-scrollbar">
        {[
          ...
          { id: "dirigentes", label: "Dirigentes (Cadastro)", icon: Users },
          { id: "unidades", label: "Unidades", icon: Building2 },
          { id: "cargos", label: "Cargos", icon: Briefcase },
          { id: "afastamentos", label: "Afastamentos", icon: Calendar }
        ].map(tab => (
          <button
            key={tab.id}
            onClick={() => setMainTab(tab.id as any)}
            className={`flex items-center gap-2 px-5 py-3 text-sm font-bold border-b-2 transition-colors whitespace-nowrap ${
              mainTab === tab.id ? "border-[#003366] text-[#003366] bg-blue-50/50" : "border-transparent text-slate-500 hover:text-slate-800 hover:bg-slate-50"
            }`}
          >
              ...
              <Card>
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-slate-50 border-b border-slate-200 text-[10px] uppercase font-black tracking-wider text-slate-500">
                        <th className="p-4">Titular Afastado</th><th className="p-4">Substituto que Assumiu</th><th className="p-4">Motivo / Tipo</th><th className="p-4">Data Início</th><th className="p-4">Data Retorno</th><th className="p-4 text-center">Ações</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100 text-sm">
                      {filteredAfastamentos.map(a => (
                        <tr key={a.id_afastamento} className="hover:bg-slate-50/50 transition-colors">
                          ...
                          <td className="p-4 text-amber-700 font-bold">{fmtDate(a.data_fim)}</td>
                          <td className="p-4 text-center">
                            <div className="flex items-center justify-center gap-1">
      ...
      {modalType === "dirigente" && (
        <Modal title={isEdit ? "Editar Cadastro de Dirigente" : "Novo Cadastro de Dirigente"} maxW="max-w-4xl" onClose={() => setModalType(null)}>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-4 h-full">
                <h4 className="text-xs font-black text-slate-500 uppercase tracking-wider flex items-center gap-1"><Users size={14}/> Dados Pessoais</h4>
                <div><label className={lbl}>Nome Completo</label><input className={inp} value={formData.nome_completo || ""} onChange={e => setFormData({...formData, nome_completo: e.target.value})} /></div>
            ...
            <div className="space-y-4">
              <div className="bg-blue-50/50 p-4 rounded-xl border border-blue-100 space-y-4">
                <h4 className="text-xs font-black text-[#003366] uppercase tracking-wider flex items-center gap-1"><Briefcase size={14}/> Dados de Exercício (Mandato)</h4>
                ...
                <div className="mb-4 bg-slate-50/80 p-3 rounded-lg border border-slate-200">
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer items-center rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2 ${formData.is_substituto ? 'bg-blue-600' : 'bg-slate-300'}`}
                    >
                  ...
                  <div className="text-[11px] text-slate-500 mt-1 ml-7">Marque esta opção se o dirigente está apenas substituindo a função.</div>
                </div>
          ...
          <button className={`${btnPrimary} w-full mt-6 py-3`} onClick={handleSubmit}>{isEdit ? "Atualizar Tudo" : "Salvar Cadastro Completo"}</button>
        </Modal>
      )}
      
      {modalType === "unidade" && (
        <Modal title={isEdit ? "Editar Unidade" : "Cadastrar Unidade"} onClose={() => setModalType(null)}>
          ...
          <button className={`${btnPrimary} w-full mt-4`} onClick={handleSubmit}>{isEdit ? "Atualizar" : "Salvar"}</button>
        </Modal>
      )}

      {modalType === "cargo" && (
        <Modal title={isEdit ? "Editar Cargo" : "Cadastrar Cargo"} onClose={() => setModalType(null)}>
      ...
      {modalType === "afastamento" && (
        <Modal title={isEdit ? "Editar Afastamento" : "Registrar Afastamento"} onClose={() => setModalType(null)}>
          <div><label className={lbl}>Mandato / Titular Afastado</label>
            <select className={inp} value={formData.id_mandato || ""} onChange={e => setFormData({...formData, id_mandato: e.target.value})}>
              ...
              {mandatos.filter(m => !m.is_substituto).map(m => (
          ...
          <div><label className={lbl}>Motivo</label><input className={inp} placeholder="Ex: Férias, Licença Médica..." value={formData.motivo || ""} onChange={e => setFormData({...formData, motivo: e.target.value})} /></div>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div><label className={lbl}>Data Fim</label><input type="date" className={inp} value={formData.data_fim || ""} onChange={e => setFormData({...formData, data_fim: e.target.value})} /></div>
          </div>
          <button className={`${btnPrimary} w-full mt-4`} onClick={handleSubmit}>{isEdit ? "Atualizar" : "Gravar"}</button>
        </Modal>
      )}
